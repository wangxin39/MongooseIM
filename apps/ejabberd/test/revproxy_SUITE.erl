...
http_upstream(_Config) ->
    %% Given
    Host = "http://localhost:8080",
    Path = <<"/abc/login.htm?ts=1231231232222&st=223232&page=32">>,
    Method = "GET",
    Headers = [{<<"host">>, <<"qwerty.com">>}],
...
qs_match(Config) ->
    ...
    Path = <<"/a/b/c/login.htm?ts=1231231232222&st=223232&page=32"
             "&ap=123442&whatever=somewordshere">>,
    Method = <<"GET">>,
...
upstream_qs(_Config) ->
    ...
    Upstream = #upstream{type = uri,
                         ...
                         path = [<<"ghi">>]},
    Path = [<<"abc">>],
    Remainder = [<<"def">>, <<"login.htm?ts=1231231232222&st=223232&page"
                              "=32&ap=123442&whatever=somewordshere">>],
    Match = #match{upstream = Upstream,
                   remainder = Remainder,
                   bindings = [],
                   path = Path},
    ...
    %% Then
    Resource = <<"/ghi/def/login.htm?ts=1231231232222&st=223232&page=32"
                 "&ap=123442&whatever=somewordshere">>,
